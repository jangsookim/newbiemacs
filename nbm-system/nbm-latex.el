...
(defun nbm-latex-toggle-LHS-RHS ()
  "Change A=B to B=A in a math environment. There must be exactly one equality = in the environment."
  (interactive)
  (let (LHS RHS LHS-beg LHS-end RHS-beg RHS-end beg end eqstr)
    (when (texmathp)
      (setq beg (nth 1 (nbm-latex-find-math-mode t)))
      (setq end (nth 2 (nbm-latex-find-math-mode t)))
      (setq eqstr (split-string (buffer-substring beg end) "="))
      (if (equal (length eqstr) 2)
	  (progn
	    (goto-char (+ beg 2))
	    (re-search-forward "[^ \t\n]") (backward-char)
	    (setq LHS-beg (point))
	...
	(message "There must be exactly one equality = in the math environment!")))
    (unless beg
