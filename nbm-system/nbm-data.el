...
newbiemacs/nbm-user-settings/nbm-variables."
  ...
  (beginning-of-buffer)
  (re-search-forward (format "^KEY=%s" (regexp-quote key)))
  (beginning-of-line)
  (kill-line) (kill-line) (save-buffer) (kill-buffer))
...
(defun nbm-visit ()
  ...
  (let (choice prompt file-list data item key)
    ...
    (dolist (item (nbm-data-get-all "visit"))
      (setq file-list (format "%s\n%s) %s" file-list (car item) (cdr item))))
    ...
    (cond ((equal choice "+")
	   ...
	   (nbm-data-add "visit" key (file-name-nondirectory (buffer-file-name))))
	  ((equal choice "-")
