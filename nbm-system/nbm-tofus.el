...
(if (fboundp 'nbm-f)
  ...
  (progn
    (defconst *tofus-game-path* (concat (getenv "HOME") "/.newbiemacs/"))
    (unless (file-exists-p *tofus-game-path*)
      (make-directory *tofus-game-path*))))
...
(defun tofus ()
  ...
  (tofus-mode)
  (tofus-init)
  (local-set-key [left] 'tofus-move-left)
  (local-set-key [right] 'tofus-move-right)
  (local-set-key [up] 'tofus-move-up)
  (local-set-key [down] 'tofus-move-down)
  (local-set-key (kbd "j") 'tofus-move-left)
  (local-set-key (kbd "l") 'tofus-move-right)
  (local-set-key (kbd "i") 'tofus-move-up)
  (local-set-key (kbd "k") 'tofus-move-down)
  (local-set-key (kbd "n") 'tofus-start-game)
  (local-set-key (kbd "r") 'tofus-resume-game)
  (local-set-key (kbd "q") 'tofus-end-game)
  (local-set-key (kbd "c") 'tofus-change-theme)
  (local-set-key (kbd "s") 'tofus-show-score-board)
  (local-set-key (kbd "p") 'tofus-pause-game))
